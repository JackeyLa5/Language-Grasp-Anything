...
class GraspNet(nn.Module):
    def __init__(self, cylinder_radius=0.05, seed_feat_dim=512, is_training=True):
        ...
        self.backbone = MinkUNet14D(in_channels=3, out_channels=self.seed_feature_dim, D=3) #输入点云 输出点的特征512维
    ...
    def forward(self, end_points):
        ...
        graspness_mask = graspness_score > GRASPNESS_THRESHOLD  #大于点的可抓阈值再FPS采样生成视图景观选择
        ...
        for i in range(B):
            ...
            cur_feat = seed_features_flipped[i][cur_mask]  # Ns*feat_dim  特征
            cur_seed_xyz = seed_xyz[i][cur_mask]  # Ns*3  坐标

            cur_seed_xyz = cur_seed_xyz.unsqueeze(0) # 1*Ns*3
            fps_idxs = furthest_point_sample(cur_seed_xyz, self.M_points)   #最远点采样最大化采样点之间的距离 采样1024个点
        ...
        end_points, res_feat = self.rotation(seed_features_graspable, end_points)  #输入可抓点信息
        ...
        # crop 输入可抓点、可抓点特征（包括点特征和视角特征）、最好的抓取视角
